...
namespace ZeyWinAds.Tests.Runtime
{
    ...
    // polling test like FcmTokenReceivedWithinBudget can get collaterally failed by the known,
    // harmless "[ZeyWinAds] Предзагрузка не удалась для баннер after 2 attempts" (no ad inventory
    // for the banner slot right now - expected, not a real defect; see AdPreload's header comment).
    // Our own Assert calls are what actually decide pass/fail in this suite, so
